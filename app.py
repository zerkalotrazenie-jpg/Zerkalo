...
def admin_geolocation(message):
    c.execute("SELECT user_id, name, last_lat, last_lon, last_seen FROM users WHERE last_lat IS NOT NULL ORDER BY last_seen DESC LIMIT 20")
    users = c.fetchall()
    if not users:
